...
export default {
  ...
  methods: {
    ...
    initEvent() {
      ...
      EventBus.$on('start-unionAnalysis', this.unionAnalysis); // 热点分析 、密度分析
      EventBus.$on('clear-analysis', this.clearAnalysis); // 清除分析
    },
    offEvent() {
      ...
      EventBus.$off('start-unionAnalysis', this.unionAnalysis); // 热点分析   密度分析
      EventBus.$off('clear-analysis', this.clearAnalysis); // 清除分析
    },
    query(q){
      this.queryc = q;
    },
    ...
    async drawSearch(layer) {
      ...
      //穿透查询 分支
      if (layer._radius && 10>=layer._radius > 0) {
        this.clearFM();
      ...
      }else if(layer._radius && layer._radius > 10){
        let radius = layer._mRadius
        let lats = layer._latlng.lat;
        let lngs = layer._latlng.lng
        var polygonC = L.polygon(createCircle(lats,lngs,radius*100000),{color: 'red'});
        // var layercircle = polygonC.addTo(map);
        // 画圆算法
        function createCircle(centerPointx, centerPointy, radius) {
          var point2Ds = [];
          var earthRadius = 5192038.0;
          var mapRadius = Angle(radius / earthRadius/1.1);
        ...
        serviceMgr.getRestResult(polygonC,this.specialLayers,res => {
        ...
        if (arr.length > 0) {
          ...
          if (type == "polygon") {
            ...
            // map.fitBounds(impGeo.getBounds());
             ...
             impGeo = L.circle(pointsList).addTo(drawLayerGroup);
          }
        }
      }
    },
      ...
      if (!this.tipTemplateName) {
        ...
        queryLayer.bindPopup(
          layer => {
            return new Vue({
              render(h) {
                return <FunctionalTable objData = {
                  layer.feature.properties
                }
                />;
              }
    ...
    heatAnalysis(heatMapData) { // 开启热力图分析
      ...
      var heatNumbers = heatMapData.coords.length,
        heatRadius = heatMapData.params.radius,
        colorParams = heatMapData.params.colorValue;
    ...
    bufferAnalysis(bufferParams) { // 缓冲区分析
      ...
      dsBufferAnalystService.bufferAnalysis(dsBufferAnalystParameters, function(serviceResult) {
        ...
        if (result.succeed) {
          ...
          self.resultLayer = L.geoJSON(result.recordset.features, {
            color: "red"
          }).addTo(self.analysisMap);
        } else {
          self._loadingService.close();
        }
      });
    ...
    unionAnalysis(unionParm) {  // 密度、热点分析
      this._loadingService = Loading.service({
        target: this.$el,
        background: "rgba(0, 0, 0, 0.8)",
        text: "拼命加载中",
      ...
      var self = this;
      var query = [];
      unionParm.queryRange.split(',').map((item) => {
        query.push(parseFloat(item));
      })
      var queryRange = L.latLngBounds(L.latLng(query[1], query[0]), L.latLng(query[3], query[2]));
      var host = "http://192.168.46.64:8090";
      var processingUrl = host + "/iserver/services/distributedanalyst/rest/v1/jobs",
        token = 'Kk98qDDFWXt3cYsa4BjjVLFWyyaaz8TeZNQ1ssosQkIYC-sBDbBQUMJgqV9DsZ9SbGuLxqASk-sJ8tJgjag6lg..';
      var processingService = new L.supermap.processingService(processingUrl);
      var params;
      var callback = function (serviceResult) {
        if (serviceResult.error) {
              self.$message.error('请求或服务器错误');
              self._loadingService && self._loadingService.close();
              return;
          }
          serviceResult.result.setting.serviceInfo.targetServiceInfos.map(function (info) {
              if (info.serviceType === 'RESTMAP') {
                  SuperMap.FetchRequest.get(info.serviceAddress + '/maps').then(function (response) {
                      ...
                      self.resultLayer = L.supermap.tiledMapLayer(mapUrl, {noWrap: true, transparent: true});
                      self.resultLayer.addTo(self.analysisMap);
                      self._loadingService && self._loadingService.close();
                  });
              }
          });
      };
      if (unionParm.flag === 'hotspot') {  // 热点分析
        params = new SuperMap.SummaryMeshJobParameter({
            datasetName: unionParm.datasetName,
            resolution: unionParm.resolution,
            meshType: unionParm.meshType,
            fields: unionParm.fields,
            query: queryRange,
            statisticModes: unionParm.statisticModes,
            type: unionParm.polyType
        }); 
        processingService.addSummaryMeshJob(params, function(serviceResult) {
          callback(serviceResult);
        });
      }
      if (unionParm.flag === 'density') {  // 密度分析
        params = new SuperMap.KernelDensityJobParameter({
          datasetName: unionParm.datasetName,
          method: unionParm.method,
          meshType: unionParm.meshType,
          fields: unionParm.fields,
          query: queryRange,
          resolution: unionParm.meshSize,
          meshSizeUnit: unionParm.meshSizeUnit,
          radius: unionParm.radius,
          radiusUnit: unionParm.radiusUnitd,
          areaUnit: unionParm.areaUnit
        });
        processingService.addKernelDensityJob(params, function(serviceResult) {
          callback(serviceResult);
        });
      }
    },
